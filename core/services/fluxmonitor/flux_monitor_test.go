...
func TestPollingDeviationChecker_PollIfEligible(t *testing.T) {
	...
	tests := []struct {
		...
		{"eligible, connected, funded, threshold > 0, answers deviate",
			true, true, true, 0.1, 200, 1, 100, true, true},
		{"eligible, connected, funded, threshold > 0, answers do not deviate",
			true, true, true, 0.1, 200, 100, 100, true, false},

		{"eligible, disconnected, funded, threshold > 0, answers deviate",
			true, false, true, 0.1, 200, 1, 100, false, false},
		{"eligible, disconnected, funded, threshold > 0, answers do not deviate",
			true, false, true, 0.1, 200, 100, 100, false, false},

		{"ineligible, connected, funded, threshold > 0, answers deviate",
			false, true, true, 0.1, 200, 1, 100, false, false},
		{"ineligible, connected, funded, threshold > 0, answers do not deviate",
			false, true, true, 0.1, 200, 100, 100, false, false},

		{"ineligible, disconnected, funded, threshold > 0, answers deviate",
			false, false, true, 0.1, 200, 1, 100, false, false},
		{"ineligible, disconnected, funded, threshold > 0, answers do not deviate",
			false, false, true, 0.1, 200, 100, 100, false, false},

		{"eligible, connected, underfunded, threshold > 0, answers deviate",
			true, true, false, 0.1, 200, 1, 100, false, false},
		{"eligible, connected, underfunded, threshold > 0, answers do not deviate",
			true, true, false, 0.1, 200, 100, 100, false, false},

		{"eligible, disconnected, underfunded, threshold > 0, answers deviate",
			true, false, false, 0.1, 1, 200, 100, false, false},
		{"eligible, disconnected, underfunded, threshold > 0, answers do not deviate",
			true, false, false, 0.1, 200, 100, 100, false, false},

		{"ineligible, connected, underfunded, threshold > 0, answers deviate",
			false, true, false, 0.1, 200, 1, 100, false, false},
		{"ineligible, connected, underfunded, threshold > 0, answers do not deviate",
			false, true, false, 0.1, 200, 100, 100, false, false},

		{"ineligible, disconnected, underfunded, threshold > 0, answers deviate",
			false, false, false, 0.1, 200, 1, 100, false, false},
		{"ineligible, disconnected, underfunded, threshold > 0, answers do not deviate",
			false, false, false, 0.1, 200, 100, 100, false, false},
	}
...
func TestOutsideDeviation(t *testing.T) {
	...
	tests := []outsideDeviationRow{
		...
		{"0 current price, outside deviation", i(0), i(100), 2, 1e307, true},
		{"0 current and next price", i(0), i(0), 2, 1e307, false},

		{"inside deviation", i(100), i(101), 2, 1e307, false},
		{"equal to deviation", i(100), i(102), 2, 1e307, true},
		{"outside deviation", i(100), i(103), 2, 1e307, true},
		{"outside deviation zero", i(100), i(0), 2, 1e307, true},

		{"inside deviation, crosses 0 backwards", f(0.1), f(-0.1), 201, 1e307, false},
		{"equal to deviation, crosses 0 backwards", f(0.1), f(-0.1), 200, 1e307, true},
		{"outside deviation, crosses 0 backwards", f(0.1), f(-0.1), 199, 1e307, true},

		{"inside deviation, crosses 0 forwards", f(-0.1), f(0.1), 201, 1e307, false},
		{"equal to deviation, crosses 0 forwards", f(-0.1), f(0.1), 200, 1e307, true},
		{"outside deviation, crosses 0 forwards", f(-0.1), f(0.1), 199, 1e307, true},
		...
		t.Run(test.name, func(t *testing.T) {
			c := func(test outsideDeviationRow) {
				actual := fluxmonitor.OutsideDeviation(test.curPrice, test.nextPrice,
					test.threshold, test.absoluteThreshold)
				assert.Equal(t, test.expectation, actual,
					"check on OutsideDeviation failed for %s", test)
			}
			c(test) // Checks on relative threshold
			// Check corresponding absolute threshold tests; make relative threshold
			// (mostly) impossible to cross
			test.threshold = 1e307
			// Initial absoluteThreshold is never crossed, but with curPrice=0, any
			// nonzero nextPrice will cross the relative threshold, yielding true
			test.expectation = test.curPrice.Equal(i(0)) && test.nextPrice.GreaterThan(i(0))
			c(test)
			test.absoluteThreshold = 1e-307
			// that absoluteThreshold is crossed, as long as prices aren't exactly equal
			test.expectation = !test.curPrice.Equal(test.nextPrice)
			c(test)
		})
	}
}
