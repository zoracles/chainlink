...
typedef struct {
    ...
    const void* data;
} secp256k1_kyber_callback;

static SECP256K1_INLINE void secp256k1_kyber_callback_call(const secp256k1_kyber_callback * const cb, const char * const text) {
...
static SECP256K1_INLINE void *checked_malloc(const secp256k1_kyber_callback* cb, size_t size) {
    void *ret = malloc(size);
    if (ret == NULL) {
        secp256k1_kyber_callback_call(cb, "Out of memory");
    }
    return ret;
}

static SECP256K1_INLINE void *checked_realloc(const secp256k1_kyber_callback* cb, void *ptr, size_t size) {
